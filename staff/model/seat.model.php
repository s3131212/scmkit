...
class Seat_Model {
    ...
    public function modify_seat($array,$width,$height,$class,$method){
        if($method == "insert") $GLOBALS['db']->insert(array("class"=>View::xss_filter($class),"data"=>json_encode(array($array)),"width"=>View::xss_filter($width),"height"=>View::xss_filter($height)),"seat"); 
        else $GLOBALS['db']->update("seat",array("data"=>json_encode(array($array)),"width"=>View::xss_filter($width),"height"=>View::xss_filter($height)),array("class"=>View::xss_filter($class))); 
    }
}