...
class Tshare_Model {
    ...
    public function insert_file($name,$filename,$passphrase,$id,$view_permission_string){
        $GLOBALS['db']->insert(array("filename"=>View::xss_filter($name),"upload_teacher"=>$id,"password"=>$passphrase,"realpath"=>$filename,"view_permission"=>$view_permission_string,"is_staff"=>1),"teacher_share");
    ...
    public function update_data($view_permission_string,$filename,$id){
        $GLOBALS['db']->update('teacher_share',array("view_permission"=>$view_permission_string,"filename"=>View::xss_filter($filename)),array("id"=>$id)); 
        return false;
    }
