...
class Communication_Model {
	...
	public function update_communication($empty,$class,$content,$date){
		...
		if($empty=="true"){
        	$result = $GLOBALS['db']->insert(array("class"=>View::xss_filter($class),"date"=>View::xss_filter($date),"content"=>View::xss_filter($content,true)),"communication");
    	}else{
        	$result = $GLOBALS['db']->update("communication",array("content"=>View::xss_filter($content,true)),array("class"=>View::xss_filter($class),"date"=>View::xss_filter($date)));
    	}
    	return $result;
	}
}