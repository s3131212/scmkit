...
class Curriculum_Controller{
    public function curriculum_ger(){
        ...
        foreach ($model->list_class() as $val) {
            $res = $model->list_curriculum($val["id"]); //借用一下輸出課表要用的method，反正要選取的資料一樣，這是抓取班級名稱用的
            $output .= '<div class="box"><h2>'.$res[0]["grade"].'年'.$res[0]["name"].'班</h2><a href="modify_curriculum.php?id='.$val['id'].'"><button>修改課表</button></a>'.$this->list_curriculum($val["id"]).'</div>';
        }
    ...
    public function update_curriculum($class,$post){
        ...
        header("Location: modify_curriculum.php?id=".$class."&s=1");
    }
}  