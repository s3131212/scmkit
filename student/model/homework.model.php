...
class Homework_Model {
	...
	public function insert_file($name,$filename,$passphrase,$user_id,$id){
        $GLOBALS['db']->insert(array("filename"=>View::xss_filter($name),"upload_student"=>View::xss_filter($user_id),"homework_id"=>$id,"password"=>$passphrase,"realpath"=>$filename),"homework_upload");
        $filedata = $GLOBALS['db']->select("student_homework", array("realpath" => $filename));
		return $filedata[0];
	}
