...
class Score_Controller {
    public function score_page(){
        echo Template::call_view("score",array("score"=>$this->list_test(),"scoresheet"=>$this->list_scoresheet()));
    }
    public function list_test($class_array = ""){
        $table = "";
        $model = new Score_Model;
        foreach ($model->select_test() as $d) {
            $table .= "<tr><td>" . $d["name"] . "</td><td><a href='score-view.php?id=" . $d["id"] . "' class='btn'><button>檢視成績</button></a></td></tr>";
        }
        return $table;
    }
    public function list_scoresheet(){
        ...
        foreach ($model->select_scoresheet_all() as $d) {
            $table .= "<tr><td>".$d["name"]."</td><td><a href='scoresheet-view.php?id=".$d["id"]."'><button>檢視成績單</button></a></tr>";
        }
        return $table;
    }
    public function view_score($id){
        ...
        foreach($table as $key => $value){
            ...
            foreach($class_score as $k => $v){
                ...
                if(checkpermission(array("student"))){
                    ...
                    if(!in_array($key,$_SESSION["class"])) $allowedit = false;
                }
                $output .= "<tr><td>". $student["name"] . "</td><td>". $v ."</td><td>". $class_rate ."</td><td>". $all_score[$k] ."</td>";
                if($allowedit){
                    $output .= "<td><a href='score-modify.php?id=".$k."&test=".$id."' class='btn' ><button ".$disabled.">編輯</button></a></td></tr>";
                }else{
                    $output .= "<td></td></tr>";
                }
                $class_rate++;
        ...
        echo Template::call_view("score-view",array("data"=>$output,"id"=>$id,"name"=>$res["name"],"js"=>$js));
    }
    ...
    public function score_new_form($type){
        ...
        $model = new Score_Model;
        if($type == "new") echo Template::call_view("score-new",array("options"=>$model->class_option()));
        elseif($type == "import") echo Template::call_view("score-import",array("options"=>$model->class_option()));
    }
    public function test_modify_form($id){
        ...
        $model = new Score_Model;
        $res = $model->select_test_by_id($id);
        echo Template::call_view("test-modify",array("test_name"=>$res["name"],"option"=>$model->class_option($res["class"]),"id"=>$id));
    }
    ...
    public function scoresheet_form($id){
        ...
        $model = new Score_Model;
        if($id == "new"){
            echo Template::call_view("scoresheet-modify",array("class"=>$model->class_option(),"test"=>$model->test_option(),"id"=>$id));
        }else{
            $res = $model->select_scoresheet($id);
            echo Template::call_view("scoresheet-modify",array("name"=>$res["name"],"class"=>$model->class_option($res["classid"]),"test"=>$model->test_option($res["testid"]),"id"=>$id));
        }
    }
    ...
    public function update_score_table($test,$id){
        ...
        echo Template::call_view("score-modify",array("id"=>$id,"test"=>$test,"student_name"=>$student["name"],"test_name"=>$res["name"],"student_score"=>$model->select_student_score($test,$id)));
    }
    ...
    public function scoresheet_view($id){
        ...
        echo Template::call_view("scoresheet-view",array("id"=>$id,"name"=>$res["name"],"data"=>$output,"id"=>$id));
    }
