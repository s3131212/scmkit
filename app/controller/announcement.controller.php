...
class Announcement_Controller {
    public function list_content($dataperpage,$nowpage){
        ...
        foreach($data[0] as $d){ 
            ...
            if(checkpermission(array("staff"))){
                $table .= "<a href=\"announcement-modify.php?id=". $d["id"] ."\" class=\"btn btn-link\">修改公告</a>
                <a href=\"announcement-modify.php?id=". $d["id"] ."&mode=delete\" class=\"btn btn-link\">刪除公告</a>";
            }
            $table .= "</div>";
        }
        $table .= $this->create_nav($dataperpage,$nowpage,$data[1]);
        echo Template::call_view("announcement",array("data"=>$table));
    }
    ...
    public function change_data_table($id = ""){
        ...
        if($id!="new"){
            ...
            echo Template::call_view("announcement-modify",array("head"=>"您正在修改「".$res["title"]."」的內容","title"=>$res["title"],"content"=>$res["content"],"id"=>$id));
        }else{
            echo Template::call_view("announcement-modify",array("head"=>"新增公告","title"=>"","content"=>"","id"=>"new"));
        }
    }
