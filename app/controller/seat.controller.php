...
class Seat_Controller {
    public function ger_seat(){
        ...
        if(checkpermission(array("student"))){
            $res = $model->get_class($_SESSION["class"]);
            $output .= "<div class='box'><h2>".$res["grade"]."年".$res["name"]."班</h2>";
            $output .= $this->list_table($_SESSION["class"]);
            $output .= "</div>";
        }elseif(checkpermission(array("teacher"))){
            foreach ($_SESSION["class"] as $value) {
                $res = $model->get_class($value);
                $output .= "<div class='box'><h2>".$res["grade"]."年".$res["name"]."班</h2> <a href='seat-modify.php?id=".$value."' style='float:right;'><button>修改座位表</button></a>";
                $output .= $this->list_table($value);
                $output .= "</div>";
            }
        }elseif(checkpermission(array("staff"))){
            foreach ($model->get_class() as $value) {
                $res = $model->get_class($value["id"]);
                $output .= "<div class='box'><h2>".$res["grade"]."年".$res["name"]."班</h2> <a href='seat-modify.php?id=".$value["id"]."' style='float:right;'><button>修改座位表</button></a>";
                $output .= $this->list_table($value["id"]);
                $output .= "</div>";
            }
        }
        echo Template::call_view("seat",array("data"=>$output));
    }
    ...
    public function list_modify_table($class){
        ...
        $table .= "</table></div>";
        echo Template::call_view("seat-modify",array("data"=>$table,"class"=>$class));
    }
    public function modify_seat($data,$class){
        ...
        header("Location: seat-modify.php?id=".$class."&s=1");
        return true;
    }
}