...
class Score_Model {
    ...
    public function select_test_all_score($id){
        $data = array();
        foreach ($GLOBALS['db']->ExecuteSQL("SELECT a.*, b.* FROM `student` as a, `scoredata` as b where b.testid = '".$id."' and a.id = b.studentid") as $k) {
    ...
    public function test_modify($name,$view_permission,$id){
        $GLOBALS['db']->update('test',array("name"=>Security::xss_filter($name),"class"=>$view_permission),array("id"=>$id));
    }
    ...
    public function scoresheet_modify($name,$class,$test,$id){
        ...
        header("Location: scoresheet-view.php?id=".$id);
    }
    ...
    public function insert_score($test_name,$view_permission_string,$score,$average,$sd){
        $GLOBALS['db']->insert(array("name"=>Security::xss_filter($test_name),"class"=>$view_permission_string,"average"=>$average,"standard_deviation"=>$sd),"test"); 
