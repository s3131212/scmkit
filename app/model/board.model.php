...
class Board_Model {
    ...
    public function list_class(){
        return $GLOBALS["db"]->select("class");
    }
    public function insert_board($title,$content,$class,$id=""){
        ...
        if($GLOBALS["db"]->insert(array("title"=>Security::xss_filter($title),"content"=>Security::xss_filter($content,1),"floors"=>$floor,"last_update"=>$time,"author"=>$_SESSION["login_id"],"author_per"=>$_SESSION["permission"],"relation"=>$re,"class"=>$class),"board")) $error=1;
        else $error=2;
        if($id!=null){
