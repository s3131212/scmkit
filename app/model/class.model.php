...
class Class_Model{
	...
	public function update_name($grade,$name,$id){
		$result = $GLOBALS['db']->update('class',array("grade"=>Security::xss_filter($grade),"name"=>Security::xss_filter($name)),array("id"=>$id));
		return $result;
	}
	public function insert_class($grade,$name){
		$result = $GLOBALS['db']->insert(array("grade"=>Security::xss_filter($grade),"name"=>Security::xss_filter($name)),"class");
		return $result;
	}
