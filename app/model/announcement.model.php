...
class Announcement_Model {
    ...
    public function modify_announcement($id = "",$content,$title,$new,$author){
        if($new){
            $result = $GLOBALS['db']->insert(array("date"=>date("Y/m/d"),"title"=>Security::xss_filter($title),"content"=>Security::xss_filter(nl2br($content),true),"author"=>$author),"announcement");
            $alert = '<div class="alert alert-success" style="display:none;">新增完成</div>';
        }else{
            $result = $GLOBALS['db']->update("announcement",array("date"=>date("Y/m/d"),"title"=>Security::xss_filter($title),"content"=>Security::xss_filter(nl2br($content),true),"author"=>$author),array("id"=>$id));
