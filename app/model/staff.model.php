...
class Staff_Model {
	...
	public function update_info($name,$login_name,$address,$phone,$id){
    	$GLOBALS['db']->update('staff',array("name"=>Security::xss_filter($name),"login_name"=>Security::xss_filter($login_name),"address"=>Security::xss_filter($address),"phone"=>Security::xss_filter($phone)),array("id"=>$id));
    	return false;
	}
	...
	public function new_staff($name,$id,$login_name,$psd){
    	...
    	if($namecheck[0]["count"] > 0){
    		...
    		$GLOBALS['db']->insert(array("name"=>Security::xss_filter($name),"id"=>$id,"login_name"=>Security::xss_filter($login_name),"password"=>md5($psd)),"staff");
