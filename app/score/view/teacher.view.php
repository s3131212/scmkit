...
class Score_View {
    public function score_page($test,$scoresheet){
        echo Template::call_view(array("appname"=>"score","file"=>"Score"),array("score"=>Score_View::list_test($test),"scoresheet"=>Score_View::list_scoresheet($scoresheet)));
    }
    public function list_test($test){
        $model = new Score_Model;
        foreach ($test as $d) {
            $table .= "<tr><td>" . $d["name"] . "</td><td><a href='scoreview?id=" . $d["id"] . "' class='btn'><button>檢視成績</button></a></td></tr>";
        }
        return $table;
    }
    public function list_scoresheet($scoresheet){
        $model = new Score_Model;
        $table = "";
        foreach ($scoresheet as $d) {
            $table .= "<tr><td>".$d["name"]."</td><td><a href='scoresheetview?id=".$d["id"]."'><button>檢視成績單</button></a></tr>";
        }
        return $table;
    }
    public function view_score($id){
        ...
        foreach($table as $key => $value){
            ...
            foreach($class_score as $k => $v){
                ...
                $output .= "<tr><td>". $student["name"] . "</td><td>". $v ."</td><td>". $class_rate ."</td><td>". $all_score[$k] ."</td><td><a href='scoremodify?id=".$k."&test=".$id."' class='btn'><button  ".$disabled.">編輯</button></a></td></tr>";
                $class_rate++;
            }
            $output .= "</table>";
        ...
        echo Template::call_view(array("appname"=>"score","file"=>"Score_view"),array("data"=>$output,"id"=>$id,"name"=>$res["name"],"js"=>$js));
    }
    public function update_score_table($test,$id){
        $model = new Score_Model;
        $res = $model->select_test_by_id($test);
        $student = $model->select_student($id);
        echo Template::call_view(array("appname"=>"score","file"=>"Score_modify"),array("id"=>$id,"test"=>$test,"student_name"=>$student["name"],"test_name"=>$res["name"],"student_score"=>$model->select_student_score($test,$id)));
    }
    public function scoresheet_form($id){
        $model = new Score_Model;
        if($id == "new"){
            echo Template::call_view(array("appname"=>"score","file"=>"Scoresheet_modify"),array("class"=>$model->class_option(),"test"=>$model->test_option(),"id"=>$id));
        }else{
            $res = $model->select_scoresheet($id);
            echo Template::call_view(array("appname"=>"score","file"=>"Scoresheet_modify"),array("name"=>$res["name"],"class"=>$model->class_option($res["classid"]),"test"=>$model->test_option($res["testid"]),"id"=>$id));
        }
    }
    ...
    public function scoresheet_view($id){
        ...
        for($i = 0;$i<count($test);$i++){
            $output .= "<td>".round($test_total[$i]/$total_student , 2)."</td>";
        }
        $output .= "<td>".$total_score."</td><td>".($total_score/($total_student*$test_counter))."</td><td></td></tr>";
        echo Template::call_view(array("appname"=>"score","file"=>"Scoresheet_view"),array("id"=>$id,"name"=>$res["name"],"data"=>$output,"id"=>$id));
    }
