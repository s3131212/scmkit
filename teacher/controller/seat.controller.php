...
class Seat_Controller {
    public function ger_seat($class_array){
        $output = "";
        $model = new Seat_Model;
        foreach ($class_array as $value) {
            $res = $model->get_class($value);
            $output .= "<div class='box'><h2>".$res["grade"]."年".$res["name"]."班</h2> <a href='modify_seat.php?id=".$value."' style='float:right;'><button>修改座位表</button></a>";
